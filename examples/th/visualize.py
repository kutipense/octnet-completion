...
def grid_wireframe(fig, ax, grid):  
  ...
  for (leaf, grid_idx, bit_idx, gd,gh,gw, bd,bh,bw, level) in pyoctnet.leaf_iterator(grid, leafs_only=False):
    ...
    if ind == 0 and leaf :
      ...
      sdf = (grid_data[data_idx + 0] * grid_data[data_idx + 1])
      if abs(sdf) < 3 and abs(sdf) > 1e-5:
        voxels[ind][x//width,z_dim-z//width,y//width] = 1
        colors[x//width,z_dim-z//width,y//width] = [abs(sdf)*0.5, abs(sdf), 3-abs(sdf)]


  color = [(0.87,0.22,0.37,0.3), (0.87,0.22,0.37,0.2), (0.87,0.22,0.37,0.1), (0.87,0.22,0.37,0.1)]
  ...
  fig.savefig("wireframe.jpg", dpi=300)
...
grid = pyoctnet.Octree.create_from_bin(b'test.oc')
grid_wireframe(fig, ax, grid)
