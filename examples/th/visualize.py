...
def grid_wireframe(ax, grid, k=0, n=0):  
  ...
  for (leaf, grid_idx, bit_idx, gd,gh,gw, bd,bh,bw, level) in pyoctnet.leaf_iterator(grid, n=n, leafs_only=False):
    ...
    if ind == 0:# and leaf:
      ...
      if abs(sdf) < 3 and abs(sdf) > 1e-5:
        voxels[ind][x//width,z_dim-z//width,y//width] = 1
        colors[x//width,z_dim-z//width,y//width] = [0, abs(sdf), 1-abs(sdf)]

  print(levels)
  color = [(0.87,0.22,0.37,0.3), (0.87,0.22,0.37,0.1), (0.87,0.22,0.37,0.1), (0.87,0.22,0.37,0.1)]
  colors = NormalizeData(colors)
  for i,voxel in enumerate(voxels):
    x,y,z = np.indices(np.array(voxel.shape) + 1)*2**i * (1 if k==1 else 1)
...
for k in range(8):    
  ...
  for i, _vx in enumerate(vxs):
    ...
    # grid_wireframe(axs[i],vx,k=i, n=k)
