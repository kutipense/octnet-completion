...
--
-- > model = require('oc_weight_init')(model, 'heuristic')
--
require("nn")
...
local function w_init(net, arg)
  ...
  for i = 1, #net.modules do
    ...
    if m.__typename == 'oc.OctreeConvolution3x3x3' then
      m:reset(method(m.nInputPlane*3*3*3, m.nOutputPlane*3*3*3))
    elseif m.__typename == 'oc.OctreeConvolutionMM' then
      m:reset(method(m.nInputPlane*3*3*3, m.nOutputPlane*3*3*3))
